...
class Tdms_read(QMainWindow, Ui_Form):
    def __init__(self, parent=None):
        ...
        self.verticalLayout.addWidget(self.canvas_tdms)
        ...
        if self.tdms_data is None:
            ...
            if self.checkBox_5_tdms.isChecked():
                ...
                if self.start >= self.end:
                    QMessageBox.information(self, self.tr("Warning"), self.tr("Time range seting wrong"),
                                            QMessageBox.Ok)
                    pass
                else:
                    self.ax_tdms.cla()
                    self.axs_tdms.cla()
                    gc.collect()
                    self.ax_tdms.plot(self.tdms_data[0, self.start:self.end], self.tdms_data[1, self.start:self.end],
                                      'k')
                    self.ax_tdms.set_xlabel('Time/s')
                    self.ax_tdms.set_ylabel('Current/nA')

                    self.axs_tdms.plot(self.tdms_data[0, self.start:self.end], self.tdms_data[2, self.start:self.end],
                                       'r')
            ...
            if self.checkBox_5_tdms.isChecked():
                ...
                if self.start >= self.end:
                    ...
                    if path[1] == 'mat (*.mat)':
                        ...
                        np.savetxt(path[0], self.part_tdms.T, delimiter=',',
                                   header='time (ms),current (nA),Voltage (V)')
                        self.statusBar().showMessage('Saved to %s' % path[0])
                        self.statusBar().showMessage(self.tr('File save success'))
                        QMessageBox.information(self, self.tr("Notice"), self.tr("File save success"), QMessageBox.Ok)
            else:
