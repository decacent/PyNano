...
def k_means_cluster(data,variety_num=2):
     #initial the center of each variety,each row of data represent one sample and the column means featuer vectors
     sample_num=np.shape(data)[0]
     #randomly select one sample as the first center
     centers=data[np.random.randint(0,len(data)-1)]
     #from the rest samples selcet one sample which with the largest euclid distance with others as another center
